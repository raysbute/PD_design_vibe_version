...
static void adminRegCreate(DepartmentNode* deptHead,
    ...
    for (RegistrationNode* r = regHead; r != nullptr; r = r->next) {
        if (r->patientID == patientID && r->departmentID == deptID
            && r->regDay == day && r->status == STATUS_PENDING) {
    ...
    cout << "\n今日(" << WEEKDAY_NAMES[week - 1] << ")该科室出诊医生:" << endl;
    printf("%-8s %-16s %-10s\n", "ID", "姓名", "级别");
    printLine(40);
    int docCount = 0;
    for (DoctorNode* d = doctorHead; d != nullptr; d = d->next) {
        if (d->departmentID == deptID && doctorWorksOnDay(d, (int)week)) {
            printf("%-8s %-16s %-10s\n", d->doctorID.c_str(),
    ...
    newNode->regID = generateRegID(day);
    newNode->patientID = patientID;
    newNode->departmentID = deptID;
    newNode->doctorID = doctorID;
    newNode->regDay = day;
    ...
    cout << "  日期: Day" << day << " (" << WEEKDAY_NAMES[week - 1] << ")" << endl;
    pauseScreen();
}
...
static void adminHospAdmit(DepartmentNode* deptHead,
    ...
    hosp->hospID = generateHospID(day);
...
static void adminHospDischarge(DepartmentNode*,
                               PatientNode* patientHead,
                               WardNode* wardHead,
                               HospitalizationNode*& hospHead) {
    printTitle("患者出院");
    ...
    int stayDays = (day > hosp->admitDay) ? (int)(day - hosp->admitDay) : 1;
...
static void adminHospViewAll(DepartmentNode*,
                             PatientNode* patientHead,
                             WardNode*,
                             HospitalizationNode* hospHead) {
    printTitle("在院患者列表");
...
static void adminTimeAdvance(PatientNode* patientHead,
                             ...
                             HospitalizationNode* hospHead) {
    printTitle("时间推进");
    cout << "当前时间: " << formatTime() << "  Day" << day << endl;
    ...
    for (unsigned int d = 0; d < daysAdvanced; d++) {
        ...
        // Expire pending registrations from days before the current day
        for (RegistrationNode* r = regHead; r != nullptr; r = r->next) {
    ...
    cout << "  当前日期: Day" << day << " (" << WEEKDAY_NAMES[week - 1] << ")" << endl;
