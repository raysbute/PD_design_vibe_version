...
bool saveAllData(
    ...
    fg << globalTime << " " << day << " " << week << " " << std::fixed << std::setprecision(2) << money << std::endl;
...
bool loadAllData(
    ...
    {
        unsigned int savedDay, savedWeek;
        if (!(fg >> globalTime >> savedDay >> savedWeek >> money)) {
            std::cout << "[警告] 全局状态文件格式错误，使用默认初始状态。" << std::endl;
            fg.close();
            return false;
        }
        setTime(globalTime);  // recalculate day and week from globalTime
        fg >> nextDeptID >> nextDoctorID >> nextPatientID >> nextRegID
