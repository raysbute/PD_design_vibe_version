...
extern unsigned int day;           // day count, derived from globalTime: day = globalTime/86400 + 1
extern unsigned int week;              // day of week, derived from day: week = ((day-1) % 7) + 1  (1=Mon..7=Sun)
extern double money;                   // hospital funds, precision to 2 decimal places

// Set the global time and automatically sync day and week from it
...
inline std::string formatTime() {
    ...
    char buf[32];
    snprintf(buf, sizeof(buf), "Day%u %02u:%02u:%02u", day, hours, minutes, seconds);
