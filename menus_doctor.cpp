...
static int showWaitingPatients(DoctorNode* doctor, RegistrationNode* regHead, PatientNode* patientHead) {
    ...
    std::cout << "当前工作日: " << WEEKDAY_NAMES[(weekday - 1) % 7] << std::endl;
    printLine(80);
    std::cout << std::left
              << std::setw(14) << "挂号ID"
              << std::setw(14) << "患者ID"
    ...
    while (cur) {
        if (cur->doctorID == doctor->doctorID && cur->status == STATUS_PENDING && cur->regDay == weekday) {
...
static void showPatientConsultations(const std::string& patientID, DoctorNode* doctor,
...
static void menuConsultPatient(DoctorNode* doctor, RegistrationNode*& regHead,
    ...
    std::cout << "就诊日期: Day" << weekday << std::endl;
...
static void menuOrderExamination(DoctorNode* doctor, ConsultationNode* consultHead,
    ...
    std::cout << "日期: Day" << weekday << std::endl;
...
bool doctorMenu(DoctorNode*& doctorHead, PatientNode* patientHead, RegistrationNode*& regHead,
    ...
    std::cout << "当前: " << formatTime() << " " << WEEKDAY_NAMES[(weekday - 1) % 7] << std::endl;
    pauseScreen();
